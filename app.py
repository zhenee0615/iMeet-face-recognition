...
def log_memory_usage(stage):
    ...
    logger.info(f"Memory Usage at {stage}: {mem} MB")
...
def perform_face_verification():
    ...
    try:
        ...
        log_memory_usage("After saving captured image")

        # Extract faces with anti-spoofing
        detected_faces = DeepFace.extract_faces(
            img_path=temp_captured_path,
        ...
        log_memory_usage("After face extraction and anti-spoofing")
        ...
        log_memory_usage("After saving profile image")
        ...
        log_memory_usage("After DeepFace verification")

        logger.info("done")
        return jsonify({
            "match": comparison_result['verified'],
            "anti_spoofing": comparison_result.get('anti_spoofing', True)  # Ensure anti-spoofing is properly implemented
        })
